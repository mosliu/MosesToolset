...
package net.liuxuan.utils.algorithm.fourier.kind3;
 ...
 */
public class FFT {   
    public static final int FFT_N_LOG = 5; // FFT_N_LOG <= 13   
...
//        float[] realIO ={18,20,19,20,19,20,18,20,18,20,19,20};
        float[] realIO =new float[FFT_N];
        System.out.println("============realIO原始值===========");
        for (int i = 0; i < realIO.length; i++) {
             realIO[i] = r.nextFloat();
             System.out.println(realIO[i]);
        }
        
        fft.calculate(realIO);
        System.out.println("============realIO后来值===========");
        for (int i = 0; i < realIO.length; i++) {
             System.out.println(realIO[i]);
        }
    }
